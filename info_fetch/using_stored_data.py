...
def get_data(author_id, file):
    ...
    if data['AuthorID'] == author_id:
        ...
        journal_names = [paper['Journal_Name'][1] for paper in data['Papers'].values()]
        unique_journal_names = list(set(journal_names))
        print('Journals:', unique_journal_names)
        print("\n \n \n")

        # List of coauthors with duplicates removed
        co_authors = [author[0] for paper in data['Papers'].values() for author in paper['Authors'] if author[1] != author_id]
        ...
        print("\n \n \n")
# Suppose your file name is data.json and you want data for AuthorID 1054758
